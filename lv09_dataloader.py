...
class SB09Data(data.Dataset):
    ...
    def __init__(self,
        ...
        self.TRAIN_IMG_PATH = os.path.join(root, 'train', 'images')
        self.TRAIN_SEG_PATH = os.path.join(root, 'train', 'labels')
        self.VAL_IMG_PATH = os.path.join(root, 'val', 'images')
        self.VAL_SEG_PATH = os.path.join(root, 'val', 'labels')
    ...
    def __getitem__(self, i): # i is index
        ...
        seg_arr = np.asarray(seg, dtype="float32") / 255
        # print('img_arr:', img_arr.shape)
        # print('seg_arr:', seg_arr.shape)

        img = np.asarray(img_arr)[:, :, 0]
        seg = np.asarray(seg_arr)[:, :, 0]
        # print('img1:', img.shape)
        # print('seg1:', seg.shape)
        img = np.asarray(img)[:, :, None]
        seg = np.asarray(seg)[:, :, None]
...
class SB09_2DLoad():
    def __init__(self, dataset, split='train', deform=True):
        super(SB09_2DLoad, self).__init__()
        self.data = []
        # self.data_image = []
        # self.data_mask = []
        self.split = split
...
if __name__ == '__main__':
    ...
    DATA_DIR = "G:\\python_project\\datasets\\SB09"
    # augs = Compose([PaddingCenterCrop(256)])
    augs = Compose([RandomSizedCrop(256), RandomHorizontallyFlip(), RandomVerticallyFlip(), RandomRotate(180)])

    dataset = SB09Data(DATA_DIR, split='val', augmentations=augs)

    sb09 = SB09_2DLoad(dataset, split='val')
    dloader = torch.utils.data.DataLoader(sb09, batch_size=1)
    for idx, batch in enumerate(dloader):
        img, mask = batch['image'], batch['mask']
        print('mask:', mask[0].shape, mask[1].shape)
        print('img:', img.shape)
